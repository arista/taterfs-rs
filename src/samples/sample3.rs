...
pub async fn sample3() -> anyhow::Result<()> {
    let repo_backend = Arc::new(fs_repo_backend::FsRepoBackend::new(fs_repo_backend::Context {
        root: PathBuf::from("/tmp/taterfs-rs-1"),
        validate_hashes_on_write: true,
    }));
    let fs = FsFileStoreService {};
    let mut iter = fs.get_file_chunks(PathBuf::from("/data/taterfs/tatercore/docs/log")).await?;
    let mut file_builder = sync_repo_file_builder::SyncRepoFileBuilder::new(sync_repo_file_builder::Context {
